...
public class IdentityService {
	 ...
	 *
	 *                            Errors to watch for: SWITCHING_PROFILES - this
	 *                            means that the FacebookLimited identity you
	 *                            provided
	 *                            already points to a different profile. You will
	 *                            likely want to offer the player the
	 *                            choice to *SWITCH* to that profile, or *MERGE* the
	 *                            profiles.
	 *
	 *                            To switch profiles, call ClearSavedProfileID() and
	 *                            call AuthenticateFacebookLimited().
	 */
	public void attachFacebookLimitedIdentity(String facebookLimitedId, String authenticationToken,
			IServerCallback callback) {
		attachIdentity(facebookLimitedId, authenticationToken, AuthenticationType.FacebookLimited, callback);
	}

	/**
	 * Merge the profile associated with the provided FacebookLimited credentials
	 * with the
	 * current profile.
	 *
	 * Service Name - Identity
	 ...
	 *                            is received
	 */
	public void mergeFacebookLimitedIdentity(String facebookLimitedId, String authenticationToken,
			IServerCallback callback) {
	 ...
	 *                          received
	 *
	 *                          Watch for DOWNGRADING_TO_ANONYMOUS_ERROR - occurs if
	 *                          you set continueAnon to false, and
	 *                          disconnecting this identity would result in the
	 *                          profile being anonymous (which means that
	 *                          the profile wouldn't be retrievable if the user
	 *                          loses their device)
	 */
	public void detachFacebookLimitedIdentity(String facebookLimitedId, boolean continueAnon,
			IServerCallback callback) {
	...
	public void attachNonLoginUniversalId(String externalId, IServerCallback callback) {
		try {
			...
			ServerCall sc = new ServerCall(ServiceName.identity, ServiceOperation.ATTACH_NONLOGIN_UNIVERSAL, data,
					callback);
			_client.sendRequest(sc);
	...
	public void updateUniversalIdLogin(String externalId, IServerCallback callback) {
		try {
			...
			ServerCall sc = new ServerCall(ServiceName.identity, ServiceOperation.UPDATE_UNIVERSAL_LOGIN, data,
					callback);
			_client.sendRequest(sc);
		} catch (JSONException je) {
			je.printStackTrace();
	...
	public void switchToParentProfile(String parentLevelName, IServerCallback callback) {
		try {
			...
			ServerCall sc = new ServerCall(ServiceName.identity, ServiceOperation.SWITCH_TO_PARENT_PROFILE, data,
					callback);
			_client.sendRequest(sc);
		} catch (JSONException je) {
			je.printStackTrace();
	...
	public void changeEmailIdentity(String oldEmailAddress, String password, String newEmailAddress,
		...
		try {
			...
			ServerCall sc = new ServerCall(ServiceName.identity, ServiceOperation.CHANGE_EMAIL_IDENTITY, data,
					callback);
			_client.sendRequest(sc);
		} catch (JSONException je) {
			je.printStackTrace();
	...
	public void attachParentWithIdentity(String externalId, String authenticationToken,
		...
		try {
			...
			ServerCall sc = new ServerCall(ServiceName.identity, ServiceOperation.ATTACH_PARENT_WITH_IDENTITY, data,
					callback);
			_client.sendRequest(sc);
		} catch (JSONException ignored) {
		}
	...
	public void attachBlockchainIdentity(String blockchainConfig, String publicKey, IServerCallback callback) {
		try {
			...
			ServerCall sc = new ServerCall(ServiceName.identity, ServiceOperation.ATTACH_BLOCKCHAIN_IDENTITY, data,
					callback);
			_client.sendRequest(sc);
		} catch (JSONException ignored) {
		}
	...
	public void detachBlockchainIdentity(String blockchainConfig, IServerCallback callback) {
		try {
			...
			ServerCall sc = new ServerCall(ServiceName.identity, ServiceOperation.DETACH_BLOCKCHAIN_IDENTITY, data,
					callback);
			_client.sendRequest(sc);
		} catch (JSONException ignored) {
		}
	...
	private void switchToChildProfile(String childProfileId,
		...
		try {
			...
			ServerCall sc = new ServerCall(ServiceName.identity, ServiceOperation.SWITCH_TO_CHILD_PROFILE, data,
					callback);
			_client.sendRequest(sc);
		} catch (JSONException je) {
			je.printStackTrace();
	...
	private void attachIdentity(String externalId, String authenticationToken, AuthenticationType authenticationType,
			IServerCallback callback) {
		try {
			JSONObject data = new JSONObject();
			data.put(Parameter.externalId.name(), externalId);
	...
	private void mergeIdentity(String externalId, String authenticationToken, AuthenticationType authenticationType,
			IServerCallback callback) {
		try {
			JSONObject data = new JSONObject();
			data.put(Parameter.externalId.name(), externalId);
