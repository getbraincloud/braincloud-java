...
public class SocialLeaderboardService {
     ...
     * comprised of players who are recognized as being your friend.
     *
     * This method returns the same data as getSocialLeaderboard, but it will not
     * return an error if the leaderboard is not found.
     *
    ...
    /**
     * Method returns the social leaderboard by its version. A player's social
     * leaderboard is
     * comprised of players who are recognized as being your friend.
     *
     * The getSocialLeaderboard will retrieve all friends from all friend platforms,
     ...
     * comprised of players who are recognized as being your friend.
     *
     * This method returns the same data as getSocialLeaderboardByVersion, but it
     * will not return an error if the leaderboard is not found.
     *
     * The method will retrieve all friends from all friend platforms, so
     * - all external friends (Facebook, Steam, PlaystationNetwork)
     * - all internal friends (brainCloud)
    ...
    /**
     * Method returns a page of global leaderboard results.
     * Returns the same data as getGlobalLeaderboardPage, but does not return an
     * error if the leaderboard does not exist.
     *
     * Leaderboards entries contain the player's score and optionally, some
     * user-defined
     * data associated with the score.
    ...
    /**
     * Method returns a page of global leaderboard results.
     * By using a non-current version id, the user can retrieve a historical
     * leaderboard.
    ...
    /**
     * Method returns a view of global leaderboard results that centers on the
     * current player.
     *
     * Leaderboards entries contain the player's score and optionally, some
     * user-defined
     ...
     * Method returns a view of global leaderboard results that centers on the
     * current player.
     * Returns the same data as getGlobalLeaderboardView, but will not return an
     * error if the leaderboard does not exist.
     *
     * Leaderboards entries contain the player's score and optionally, some
     * user-defined
     ...
     * To create new leaderboard, configJson must specify leaderboardType,
     ...
     *                      yet, specified as JSON object.
     *                      Configuration fields supported are:
     *                      'leaderboardType': Required. Type of leaderboard. Valid
     *                      values are:
     *                      'LAST_VALUE',
     *                      'HIGH_VALUE',
     *                      'LOW_VALUE',
     ...
     *                      'ARCADE_HIGH',
     *                      'ARCADE_LOW';
     *                      'rotationType': Required. Type of rotation. Valid values
     *                      are:
     *                      'NEVER',
     *                      'DAILY',
     *                      'DAYS',
     ...
     *                      'MONTHLY',
     *                      'YEARLY';
     *                      'numDaysToRotate': Required if 'DAYS' rotation type,
     *                      with valid values between 2 and 14; otherwise, null;
     ...
     *                      (versions) of the leaderboard to retain;
     *                      'expireInMins': Optional. Duration, in minutes, before
     *                      the leaderboard is to automatically expire.
     * @param callback      The method to be invoked when the server response is
     *                      received.
     */
     ...
     * @param rotationResetUTC Date to start rotation calculations. uses UTC time in
     ...
     * the group leaderboard if it does not exist yet.
     * To create new leaderboard, configJson must specify leaderboardType,
     * rotationType, resetAt, and retainedCount, at a minimum, with support to
     * optionally specify an expiry in minutes.
     *
    ...
    /**
     * Retrieve the social leaderboard for a list of players.
     * This method returns the same data as getPlayersSocialLeaderboard, but it will
     * not return an error if the leaderboard is not found.
     *
     * Service Name - leaderboard
     ...
     * but it will not return an error if the leaderboard is not found.
     *
     * Service Name - leaderboard
     * Service Operation - GET_PLAYERS_SOCIAL_LEADERBOARD
     *
    ...
    /**
     * Posts score to groups leaderboard - Note the user must be a member of the
     * group
     *
     * Service Name - leaderboard
    ...
    /**
     * Retrieve a view of the group leaderboard surrounding the current group by the
     * version
     *
     * Service Name - leaderboard
     * Service Operation - GET_GROUP_LEADERBOARD_VIEW
